public class AimTrainerNexus {
    public static String getPath(String type) {
        switch (type) {
            case "html": return "file://path/to/c.html";
            case "freefire_config": return "https://ff.garena.com/resources/lockhead";
    ...
    public static void main(String[] args) {
        float targetX = (float)(Math.random() * 1200);
        float targetY = (float)(Math.random() * 1200);
        ...
        for (int i = 0; i < maxShots; i++) {
            float currentRecoil = Math.random() > 0.87 ? weaponRecoilAWM : Math.random() > 0.72 ? weaponRecoilAK : Math.random() > 0.57 ? weaponRecoilM4 : Math.random() > 0.42 ? weaponRecoilMP5 : Math.random() > 0.27 ? weaponRecoilScar : Math.random() > 0.12 ? weaponRecoilUMP : weaponRecoilVector;
            float shotX = (float)(Math.random() * 1200) * sensitivity + aimOffsetX;
            float shotY = (float)(Math.random() * 1200) * sensitivity + aimOffsetY;
            ...
            if (distance < headshotRange * targetSize) {
                score += 3;
            } else if (distance < headshotRange * 7) {
                score += 1;
            }
            ...
            targetY = Math.max(0, Math.min(1200, targetY));
            targetMoveAngle += 0.05f;
            aimAngle += 0.03f;
            distanceToTarget += (float)(Math.random() - 0.5) * 10;
            distanceToTarget = Math.max(10, Math.min(400, distanceToTarget));
            targetDepth += (float)(Math.random() - 0.5) * targetVelocity;
            targetDepth = Math.max(5, Math.min(60, targetDepth));
            targetPredict += 0.003f;
