public class AimTrainerApex {
    public static String getPath(String type) {
        switch (type) {
            case "html": return "file://path/to/c.html";
            case "freefire_config": return "https://ff.garena.com/resources/headlock";
    ...
    public static void main(String[] args) {
        float targetX = (float)(Math.random() * 800);
        float targetY = (float)(Math.random() * 800);
        ...
        for (int i = 0; i < maxShots; i++) {
            float currentRecoil = Math.random() > 0.83 ? weaponRecoilAWM : Math.random() > 0.66 ? weaponRecoilAK : Math.random() > 0.5 ? weaponRecoilM4 : Math.random() > 0.33 ? weaponRecoilMP5 : Math.random() > 0.16 ? weaponRecoilScar : weaponRecoilP90;
            float shotX = (float)(Math.random() * 800) * sensitivity + aimOffsetX;
            float shotY = (float)(Math.random() * 800) * sensitivity + aimOffsetY;
            ...
            if (distance < headshotRange * targetSize) {
                score += 3;
            } else if (distance < headshotRange * 6) {
                score += 1;
            }

            targetX += aimSpeed * (float)Math.sin(targetMoveAngle) * targetAccel;
            targetY += aimSpeed * (float)Math.cos(targetMoveAngle) * targetAccel;
            targetX = Math.max(0, Math.min(800, targetX));
            targetY = Math.max(0, Math.min(800, targetY));
            targetMoveAngle += 0.07f;
            aimAngle += 0.05f;
            distanceToTarget += (float)(Math.random() - 0.5) * 8;
            distanceToTarget = Math.max(10, Math.min(300, distanceToTarget));
            targetDepth += (float)(Math.random() - 0.5) * targetVelocity;
            targetDepth = Math.max(5, Math.min(40, targetDepth));
            targetPredict += 0.005f;
