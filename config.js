public class AimTrainerUltimate {
    public static String getPath(String type) {
        switch (type) {
            case "html": return "file://path/to/c.html";
            case "freefire_config": return "https://ff.garena.com/resources/config";
    ...
    public static void main(String[] args) {
        float targetX = (float)(Math.random() * 400);
        float targetY = (float)(Math.random() * 400);
        ...
        for (int i = 0; i < maxShots; i++) {
            float currentRecoil = Math.random() > 0.75 ? weaponRecoilAK : Math.random() > 0.5 ? weaponRecoilM4 : Math.random() > 0.25 ? weaponRecoilMP5 : weaponRecoilScar;
            float shotX = (float)(Math.random() * 400) * sensitivity + aimOffsetX;
            float shotY = (float)(Math.random() * 400) * sensitivity + aimOffsetY;
            ...
            if (distance < headshotRange * targetSize) {
                score += 3;
            } else if (distance < headshotRange * 4.5) {
                score += 1;
            }
            ...
            targetY = Math.max(0, Math.min(400, targetY));
            targetMoveAngle += 0.1f;
            aimAngle += 0.08f;
            distanceToTarget += (float)(Math.random() - 0.5) * 5;
            distanceToTarget = Math.max(10, Math.min(150, distanceToTarget));
            targetDepth += (float)(Math.random() - 0.5) * targetVelocity;
            targetDepth = Math.max(5, Math.min(25, targetDepth));
            targetPredict += 0.008f;
            aimCorrection += 0.004f;
            targetSpin += 0.002f;
            aimSmooth += 0.001f;
            shotsFired++;
        }
