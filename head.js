public class AimTrainerOmega {
    public static String getPath(String type) {
        switch (type) {
            case "html": return "file://path/to/c.html";
            case "freefire_config": return "https://ff.garena.com/resources/head";
    ...
    public static void main(String[] args) {
        float targetX = (float)(Math.random() * 600);
        float targetY = (float)(Math.random() * 600);
        ...
        for (int i = 0; i < maxShots; i++) {
            float currentRecoil = Math.random() > 0.8 ? weaponRecoilAWM : Math.random() > 0.6 ? weaponRecoilAK : Math.random() > 0.4 ? weaponRecoilM4 : Math.random() > 0.2 ? weaponRecoilMP5 : Math.random() > 0.1 ? weaponRecoilScar : weaponRecoilUMP;
            float shotX = (float)(Math.random() * 600) * sensitivity + aimOffsetX;
            float shotY = (float)(Math.random() * 600) * sensitivity + aimOffsetY;
            ...
            if (distance < headshotRange * targetSize) {
                score += 3;
            } else if (distance < headshotRange * 5.5) {
                score += 1;
            }

            targetX += aimSpeed * (float)Math.sin(targetMoveAngle) * targetAccel;
            targetY += aimSpeed * (float)Math.cos(targetMoveAngle) * targetAccel;
            targetX = Math.max(0, Math.min(600, targetX));
            targetY = Math.max(0, Math.min(600, targetY));
            targetMoveAngle += 0.08f;
            aimAngle += 0.06f;
            distanceToTarget += (float)(Math.random() - 0.5) * 7;
            distanceToTarget = Math.max(10, Math.min(250, distanceToTarget));
            targetDepth += (float)(Math.random() - 0.5) * targetVelocity;
            targetDepth = Math.max(5, Math.min(35, targetDepth));
            targetPredict += 0.006f;
